...
# -------------------------------------------------------------------------------------------------------------------- #
    ...
    def get_image(self, resolucao: str, satelite: str, instrumento: str,
                  abrangencia: str, filtro=None, banda=None):
        goes = go.GOESObserver(satelite=satelite,
                               instrumento=instrumento,
                               abrangencia=abrangencia,
                               filtro=filtro, banda=banda)
        goes.set_imagery(resolucao=resolucao)
        self.diretorio = goes.diretorio
        self.link_img = goes.df_imagens[0]
        self.data_recente = goes.df_imagens[1]
        self.tamanho = round(goes.df_imagens[2] / (1024 * 1024), 3)
        self.working = True
        status_code = requests.get(url=self.link_img, timeout=5).status_code
        if status_code == 200:
            self.working = True
...
# -------------------------------------------------------------------------------------------------------------------- #
    ...
    def update_img(self):
        if self.working:
            ...
            wallpaper.save('/home/ego/Pictures/Wallpapers/wallpaper.jpg')
